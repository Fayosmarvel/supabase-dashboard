...
# change if you prefer a different bucket name
BUCKET_NAME = os.getenv("SUPABASE_BUCKET", "id_uploads")

if not SUPABASE_URL or not SUPABASE_KEY:
    st.error("SUPABASE_URL or SUPABASE_KEY missing in .env. Add them and restart.")
    st.stop()
...
def _parse_supabase_response(res):
    ...
    Works with objects that have .data/.error, .status_code/.data/.json, or plain dicts.
    """
    try:
...
def upload_id_image_to_storage(uploaded_file, path: str):
    ...
    Returns (ok: bool, public_url_or_error)
    """
    try:
        file_bytes = uploaded_file.getvalue()
        # Some supabase clients accept bytes, some require a file-like object.
        # We attempt bytes first, else try file-like wrapper.
        try:
            res = supabase.storage.from_(BUCKET_NAME).upload(path, file_bytes)
        except Exception:
            # fallback: upload using file-like:
            file_obj = io.BytesIO(file_bytes)
            res = supabase.storage.from_(BUCKET_NAME).upload(path, file_obj)

        ok, data, err = _parse_supabase_response(res)
        if not ok:
            # If upload returns data on success but status in dict, handle that too
            # Try to get public URL anyway
            # Build public URL using storage API (may be necessary if client didn't return public URL)
            pass
        ...
        try:
            ...
            if ok2:
                # data2 may be dict {'publicUrl': '...'} or {'publicURL': '...'} or direct string
                if isinstance(data2, dict):
                    return True, data2.get("publicUrl") or data2.get("publicURL") or data2.get("public_url") or str(data2)
                return True, data2
            else:
                # client's get_public_url may return a dict with 'publicUrl' nested inside
...
def create_auth_user(email: str, password: str):
    """
    Create a Supabase Auth user via sign_up. Returns (ok, payload_or_error)
    On success payload usually contains a 'user' and/or 'data' structure depending on client.
    """
    try:
        # many clients support dictionary param
        res = supabase.auth.sign_up({"email": email, "password": password})
    except Exception:
        # some clients take keyword args
        try:
            res = supabase.auth.sign_up(email=email, password=password)
        except Exception as e:
...
st.markdown(
    "This app uses **Supabase Auth** to create user accounts (so you don't store passwords). "
    "ID images are uploaded to Supabase Storage and only the public URL / path is stored in the `users` table."
)

col1, col2 = st.columns([1, 1])

with col1:
    st.header("Create account (Supabase Auth)")
        ...
        if errors:
            ...
            with st.spinner("Creating account..."):
                ...
                if not ok:
                    ...
                    auth_user_id = None
                    if isinstance(payload, dict):
                        # common shapes: {'user': {...}} or {'data': {'user': {...}}} or direct list
                        if "user" in payload and isinstance(payload["user"], dict):
                            ...
                            # look deeper for a user dict
                            for v in payload.values():
                                if isinstance(v, dict) and "id" in v:
                                    auth_user_id = v.get("id")
                                    break
                    # If still unknown, create a syntactic id (not ideal)
                    if not auth_user_id:
                        auth_user_id = str(uuid4())

                    # 2) Upload ID image to Storage
                    # build a path: users/{auth_user_id}/{timestamp}_{filename}
                    ...
                    if not ok_upload:
                        ...
                        # 3) Save metadata to users table
                        ok_meta, meta_resp = save_metadata_to_table(email.strip(), auth_user_id, is_18_or_older, path, image_public_url)
                        if not ok_meta:
                            st.error(f"Failed to save user metadata: {meta_resp}")
                        else:
                            st.success("Account created (Auth) and metadata saved!")

with col2:
    st.header("Dashboard (users)")
    # Fetch and display rows from users table
    try:
    ...
    # Recent table and preview
    st.subheader("Recent signups")
    if users_df.empty:
        st.info("No user metadata found.")
    else:
        ...
        latest_with_image = users_df[users_df["id_image_public_url"].notnull() & (users_df["id_image_public_url"] != "")] if "id_image_public_url" in users_df.columns else pd.DataFrame()
        if not latest_with_image.empty:
            latest_url = latest_with_image.iloc[0]["id_image_public_url"]
            try:
                st.image(latest_url, caption="Latest uploaded ID", use_column_width=True)
            except Exception:
                st.text("Could not load latest image preview from storage URL.")
        else:
            st.info("No ID images to preview.")

st.markdown("---")
st.caption(
    "Notes: Supabase may require email confirmation depending on your Auth settings. "
    "For production, restrict who can call server-side keys, enable Row Level Security and policies, and store minimal data."
)
